...
async function loadVerificationColumns() {
  ...
  const shipIdMeta = result.rows.find(
    (row) => row.column_name.toLowerCase() === "ship_id",
  );
...
function isNumericShipIdColumn() {
  return (
    shipIdDataType === "integer" ||
    shipIdDataType === "bigint" ||
    shipIdDataType === "smallint"
  );
}

function getDbShipIdForAppShipId(appShipId) {
  if (
    typeof appShipId === "string" &&
    appShipId.startsWith(SCENARIO_SHIP_ID_PREFIX)
  ) {
    const num = Number(appShipId.slice(SCENARIO_SHIP_ID_PREFIX.length));
    if (Number.isFinite(num))
      return isNumericShipIdColumn() ? num : String(num);
  }
...
async function bootstrapVerificationRows() {
  ...
  for (const ship of MOCK_SHIPS) {
    ...
    const insertColumns = [
      quoteIdentifier(shipIdColumn),
      quoteIdentifier(verifiedColumn),
    ];
    ...
    if (destinationColumn) {
      ...
      insertValues.push(
        ship.destination === "Unknown" ? null : ship.destination,
      );
...
app.patch("/api/verifications/:shipId", async (req, res) => {
  ...
  try {
    ...
    if (!shipIdColumn || !verifiedColumn) {
      return res
        .status(500)
        .json({ error: "Verification table is missing required columns." });
    }
    ...
    if (updateParts.length === 0) {
      return res
        .status(400)
        .json({ error: "No writable columns were provided." });
    }
