...
function createTriangleIcon(heading, isSelected, statusColor) {
  const size = isSelected ? 20 : 16;
  const half = size / 2;
  const fill = isSelected ? FILL_SEL : FILL;
  const sw = isSelected ? 2.2 : 1.8;

  const svg = `<svg width="${size}" height="${size}" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" overflow="visible">
    <g transform="rotate(${heading}, 9, 9)">
      <path d="M 9,2 C 11.5,5 14,10 13.5,14 C 13,15.5 11,14.5 9,12.5
        ...
        fill="${fill}" stroke="${statusColor}" stroke-width="${sw}" stroke-linejoin="round" stroke-linecap="round"/>
    </g></svg>`;
...
function createHullIcon(heading, isSelected, statusColor) {
  const size = isSelected ? 38 : 32;
  const half = size / 2;
  const fill = isSelected ? FILL_SEL : FILL;
  const sw = isSelected ? 2.4 : 2.0;

  const rot = heading - 90;
  const svg = `<svg width="${size}" height="${size}" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg" overflow="visible">
...
export default function ShipMarker({ ship, isSelected, onSelect }) {
  ...
  const statusLevel = getStatusLevel(ship);
  const statusColor = STATUS_COLORS[statusLevel];

  const icon = useMemo(
    () =>
      ship.markerType === "hull"
        ? createHullIcon(headingRounded, isSelected, statusColor)
        : createTriangleIcon(headingRounded, isSelected, statusColor),
    [headingRounded, ship.markerType, isSelected, statusColor],
  );
